...
const ResultsPage = () => {
	...
	useEffect(() => {
		async function loadReactions() {
			...
			const reacterLeaderboard = await getLeaderboard(
				client,
				`${name}-reacters`
			)
	...
	const getLeaderboard = async (cacheClient, leaderboardName) => {
		let board
		const leaderboardResponse = await cacheClient.sortedSetFetchByRank(
			process.env.NEXT_PUBLIC_CACHE_NAME,
			leaderboardName,
			{ startRank: 0, order: 'DESC' }
		)
