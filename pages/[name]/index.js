		...
		async function subscribeForReactions() {
			...
			if (!reactionSubscription) {
				const subscription = await topics.subscribe(
					...
					{
						onItem: (message) => {
							sendReaction(message.valueString())
						},
						onError: (err) => {
							console.error(err)
						},
	...
	const sendReaction = async (message) => {
		try {
			...
			if (data.type == 'reaction') {
				...
				await cacheClientRef.current.sortedSetIncrementScore(
					process.env.NEXT_PUBLIC_CACHE_NAME,
					`${name}-reacters`,
					data.username
				)
				...
				{!isSuccess ? (
					...
					<Flex direction="column" gap="1em" width="100%" alignItems="center">
						...
						<Card
							...
							borderRadius="medium"
							width="99%"
							padding="relative.small"
						>
							<Flex direction="row" justifyContent="space-between">
								<Flex direction="row" alignItems="center">
									<Link href={`${router.asPath}/react`}>
										<QRCode
											value={`https://${process.env.NEXT_PUBLIC_DOMAIN_NAME}${router.asPath}/react`}
											size={384}
											style={{ height: 'auto', maxWidth: '5em' }}
										/>
									</Link>
